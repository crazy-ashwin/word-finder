...
export default function WordsByLengthStartingWithPage({
	...
	return (
		<div className='min-h-screen bg-gradient-to-b from-gray-50 to-white py-8'>
			<head>
				<title>{wordLength} Letter Words Starting with {letter.toUpperCase()}</title>
				<meta
					name='description'
					content={`Find ${wordLength} letter words that start with ${letter.toUpperCase()}`}
				/>
				<meta
					name='keywords'
					content={`${wordLength} letter words, words starting with ${letter.toUpperCase()}, crossword solver, word games, word puzzles, word search`}
				/>
				<meta name='robots' content='index, follow' />
				<link rel='canonical' href={`${baseUrl}/words-by-length/${wordLength}/starting-with/${letter.toUpperCase()}`} />
			</head>
