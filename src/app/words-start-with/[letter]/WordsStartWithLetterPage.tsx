...
export default function WordsStartWithLetterPage({ letter }: Props) {
	...
	const [sidebarStarts, setSidebarStarts] = useState(
		searchParams.get("starts") || letter
	);
	...
	const startsWith = searchParams.get("starts")?.toLowerCase() || letter.toLowerCase();
		...
		const fetchWords = async () => {
			...
			try {
				...
				let realWords = data
					...
					.filter((w: string) => w.startsWith(startsWith));
					...
					<div className='lg:col-span-3 space-y-8'>
						...
						<div className='bg-white rounded-xl shadow-lg p-8 border border-gray-100'>
							<h1 className='text-4xl font-bold text-gray-800 mb-4'>
								{endsWith &&
									`Words Starting with "${letter.toUpperCase()}" Ending with "${endsWith.toUpperCase()}"`}
								{!endsWith &&
							...
							<p className='text-gray-700 leading-relaxed mb-4'>
								{endsWith && (
									<>
										<strong>
											...
											{letter.toUpperCase()} and end with{" "}
										...
										. Try our{" "}
										<Link
											href={`/words-by-length/5/starting-with/${letter}/ending-in/${endsWith}`}
											className='text-blue-600 hover:underline'>
											five letter words starting with{" "}
											{letter.toUpperCase()} ending with{" "}
								...
								{!endsWith && (
									<>
										...
										. Try our{" "}
										<Link
											href={`/words-by-length/5/starting-with/${letter}`}
											className='text-blue-600 hover:underline'>
											five letter words starting with{" "}
						...
						<div className='bg-white rounded-xl shadow-lg p-8 border border-gray-100'>
							<h2 className='text-2xl font-bold text-gray-800 mb-4 flex items-center gap-2'>
								...
								Friends that starts with {letter.toUpperCase()}?
							</h2>
							<p className='text-gray-700 mb-4'>
								The highest scoring word in Words With Friends
								that starts with {letter.toUpperCase()} is{" "}
						...
						<div className='bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100'>
							<div className='bg-gradient-to-r from-green-400 to-green-500 text-white px-6 py-4'>
								<h2 className='text-xl font-bold'>
									Words starting with {letter.toUpperCase()} ending in
								</h2>
							</div>
							<div className='p-6'>
								<div className='grid grid-cols-6 md:grid-cols-8 lg:grid-cols-13 gap-2'>
									{alphabet.map((endLetter) => (
										<Link
											key={endLetter}
											href={`?ends=${endLetter}`}
						...
						<div className='bg-white rounded-xl shadow-lg p-8 border border-gray-100'>
							<h2 className='text-2xl font-bold text-gray-800 mb-4 flex items-center gap-2'>
								...
								Most Common Words Starting with {letter.toUpperCase()}
							</h2>
							<p className='text-gray-700 mb-4'>
								The most common words starting with {letter.toUpperCase()} are{" "}
						...
						<div className='bg-white rounded-xl shadow-lg p-8 border border-gray-100'>
							<h2 className='text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2'>
								...
								Words Starting with {letter.toUpperCase()} FAQ
							</h2>

							<div className='space-y-4'>
								<div>
									<h3 className='font-bold text-gray-800 mb-2'>
										How many words start with {letter.toUpperCase()}?
									</h3>
									<p className='text-gray-700'>
										There are approximately {words.length}+
										common words that start with {letter.toUpperCase()} in the
								...
								<div>
									<h3 className='font-bold text-gray-800 mb-2'>
										What are the best words starting with {letter.toUpperCase()} for Scrabble?
									</h3>
									<p className='text-gray-700'>
										The best words starting with {letter.toUpperCase()} for
